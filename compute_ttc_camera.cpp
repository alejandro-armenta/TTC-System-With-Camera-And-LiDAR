                      ...
                      std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC)
{
    ...
    {
        double median_0 = distRatios[static_cast<int>(distRatios.size() / 2) - 1];
        double median_1 = distRatios[static_cast<int>(distRatios.size() / 2)];
        
        medianDistRatio = (median_0 + median_1) / 2.0;
    }
    else
    {
        medianDistRatio = distRatios[static_cast<int>(distRatios.size() / 2)];
    }
