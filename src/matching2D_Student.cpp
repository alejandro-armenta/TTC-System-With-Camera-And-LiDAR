                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    { // k nearest neighbors (k=2)
        ...
        // 2 matches per source descriptor!
        matcher->knnMatch(descSource, descRef, knn_matches, 2);

        double differenceRatio = 0.8;

        for(auto it = knn_matches.begin(); it != knn_matches.end(); ++it)
        {
            if((*it)[0].distance < differenceRatio * (*it)[1].distance)
            {
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType)
{
    ...
    else if(descriptorType.compare("ORB") == 0)
    {
        ...
        int nLevels = 8; //this will be size / (1.2 ^ nlevels);
        int edgeThreshold = 31;

        int patchSize = 31;

        int firstLevel = 0; // first image will be source image!
    ...
    else if(descriptorType.compare("AKAZE") == 0)
    {
        ...
        cv::KAZE::DiffusivityType diffusivity = cv::KAZE::DiffusivityType::DIFF_PM_G2;
    ...
    else if(descriptorType.compare("SIFT") == 0)
    {
        ...
        double edgeThreshold = 10; //More features are retained
        double sigma = 1.6; //NOTE: Reduce this?
...
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    //I guess this is sobel window size!
    int gradiantSize = 3;
    bool useHarrisDetector = true;
    double k = 0.04;

  	cv::Ptr<cv::FeatureDetector> HarrisDet = cv::GFTTDetector::create(maxCorners, qualityLevel, minDistance, blockSize, gradiantSize, useHarrisDetector,k);

    double t = (double)cv::getTickCount();
  	HarrisDet->detect(img, keypoints);
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    cout << "Harris keypoint detection in " << 1000 * t / 1.0 << " ms" << endl;
...
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, bool bVis)
{
    ...
    if(detectorType.compare("FAST") == 0)
    {
        ...
        cv::FastFeatureDetector::DetectorType fastDetectorType = cv::FastFeatureDetector::DetectorType::TYPE_9_16;

        detector = cv::FastFeatureDetector::create(threshold, nonMaxSuppression, fastDetectorType);
    }
